...
----------------------------------------------------------------------
-- DEFAULT arguments 
DROP FUNCTION IF EXISTS f(text, text, text);
CREATE FUNCTION f(x text , y text DEFAULT 'boran', z text DEFAULT 'duygus') RETURNS text AS
$$
...
SELECT f(x => 'x', z => 'z', y => 'y');  -- call function specifiying argument names
...
$$
DECLARE y x%TYPE;   -- type of y will be the same as x's type (say τ)
BEGIN
